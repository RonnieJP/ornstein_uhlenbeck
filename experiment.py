...
thresholds = np.arange(1, 2.5, 0.1)
sharpe_ratios = [strategy.sharpe_ratio(test, estimated_params, t) for t in thresholds]
t_to_s = pd.DataFrame(zip(thresholds, sharpe_ratios))
t_to_s.columns = ["Threshold Values", "Sharpe Ratios"]
fig = px.bar(t_to_s, x = "Threshold Values", y = "Sharpe Ratios", title = "Sharpe Ratios against varying Deviation Thresholds")
fig.show()